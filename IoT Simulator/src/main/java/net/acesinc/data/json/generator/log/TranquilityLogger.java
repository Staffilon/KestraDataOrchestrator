 ...
 *
 *Un produttore di Tranquility invia eventi json utilizzando Tranquility a un cluster di Druid . 
 *Druid � un archivio dati Open Source Analytics e Tranquility � un trasporto di comunicazione in tempo reale che Druid supporta 
 *per l'inserimento dei dati. Configuralo in questo modo:
{
    "type": "tranquility",
...
Quando inviamo dati a Druid tramite Tranquility, inviamo un compito al nodo Druid Overlord che
eseguir� l'importazione in tempo reale. 
La nostra implementazione delle attivit� attualmente non consente agli utenti di specificare gli aggregatori utilizzati. 
Creiamo una singola metrica "eventi" che � un Count Aggregation. 
Tutto il resto � configurato tramite le propriet�. 
Le propriet� sync e flatten si comportano esattamente allo stesso modo del produttore Kafka. 

Quando avvii il generatore, contatter� il Druid Overlord e creer� un'attivit� per la tua origine dati, quindi inizier� lo streaming dei dati in Druid.
 * @author andrewserff
 */
public class TranquilityLogger extends AbstractEventLogger {
    ...
    public TranquilityLogger(Queue<String> queue, Map<String, Object> props) {
    	super(queue);
    ...
    private void logEvent(String event) {
        try {
            ...
            addEventTrace(String.format("%s %s: Sending event to  %s", LocalDateTime.now(), "tranquillity", output));
